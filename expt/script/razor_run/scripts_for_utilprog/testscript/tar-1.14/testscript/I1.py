...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'bigfile.tar ./')
    train_run("-xvf", 'bigfile.tar')
    execute('rm -fr bigfile.tar bigfile')

    #1
    execute('cp -r ' +indir+ 'bigfile.tar ./')
    execute('mkdir tmp')
    train_run("-xvf", 'bigfile.tar -C tmp')
    execute('rm -fr bigfile.tar tmp bigfile')

    #2
    execute('cp -r ' +indir+ 'Music.tar ./')
    train_run("-x -f", 'Music.tar "new/one.mp3"')
    execute('rm -fr Music.tar new')

    #3
    execute('cp -r ' +indir+ 'Music.tar ./')
    train_run("-xv -f", 'Music.tar "new/two.mp3" "new/three.mp3"')
    execute('rm -fr Music.tar new')

    #4
    execute('cp -r ' +indir+ 'abc.tar ./')
    train_run("-t -f", 'abc.tar')
    execute('rm -fr abc.tar')

    #5
    execute('cp -r ' +indir+ 'Music.tar ./')
    train_run("-tv -f", 'Music.tar')
    execute('rm -fr Music.tar')

    #6
    execute('cp -r ' +indir+ 'Latest ./')
    train_run("-cvf Videos.tar", './Latest/')
    execute('rm -fr Latest Videos.tar')

    #7
    execute('cp -r ' +indir+ 'Videos ./')
    train_run_for_i7()
    execute('rm -fr Videos hugefie.tar')

    #8
    execute('cp -r ' +indir+ 'hugefie.tar ./')
    train_run("-t -f", 'hugefie.tar')
    execute('rm -fr hugefie.tar')

    #9
    execute('cp -r ' +indir+ 'Book.tar ./')
    execute('cp -r ' +indir+ 'pageone.txt ./')    
    train_run("-rv -f", 'Book.tar pageone.txt')
    execute('rm -fr Book.tar pageone.txt')

    #10
    execute('cp -r ' +indir+ 'Institution.tar ./')
    execute('cp -r ' +indir+ 'College ./')    
    train_run("-rf Institution.tar", './College/Engineering/top.ppt')
    execute('rm -fr Institution.tar College')
