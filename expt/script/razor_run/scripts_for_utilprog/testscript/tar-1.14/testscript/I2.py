...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'ubuntu/stuff ./')
    train_run("-cvf", 'archive.tar stuff')
    execute('rm -fr archive.tar stuff')

    #1
    train_run("-cvf archive.tar", '/usr/local/mytest')
    execute('rm -fr archive.tar')

    #2
    execute('cp -r ' +indir+ 'ubuntu/Downloads ./')
    execute('cp -r ' +indir+ 'ubuntu/Documents ./')
    train_run("-cvf archive.tar", currdir+'/Downloads /usr/local/mytest '+currdir+'/Documents')
    execute('rm -fr archive.tar Downloads Documents')

    #3
    execute('cp -r ' +indir+ 'ubuntu ./')
    train_run("-cvf archive.tar", currdir+'/ubuntu --exclude='+currdir+'/ubuntu/Downloads --exclude='+currdir+'/ubuntu/.cache')
    execute('rm -fr archive.tar ubuntu')

    #4
    execute('cp -r ' +indir+ 'ubuntu ./')
    train_run("-cvf archive.tar", currdir+'/ubuntu --exclude=*.mp4')
    execute('rm -fr archive.tar ubuntu')

    #5
    execute('cp -r ' +indir+ 'archive.tar ./')
    train_run("-xvf", 'archive.tar')
    execute('rm -fr archive.tar stuff')

    #6
    execute('cp -r ' +indir+ 'archive.tar ./')
    execute('mkdir tmp')
    train_run("-xvf", 'archive.tar -C tmp')
    execute('rm -fr archive.tar tmp')
