...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'os2.c ./')
    execute('cp -r ' +indir+ 'os3.c ./')
    execute('cp -r ' +indir+ 'os4.c ./')
    train_run("cvf file.tar", 'os2.c os3.c os4.c')
    execute('rm -fr file.tar os2.c os3.c os4.c')

    #1
    execute('cp -r ' +indir+ 'file.tar ./')
    train_run("xvf", 'file.tar')
    execute('rm -fr file.tar os2.c os3.c os4.c')

    #2
    execute('cp -r ' +indir+ 'file.tar ./')
    train_run("tvf", 'file.tar')
    execute('rm -fr file.tar')

    #3
    execute('cp -r ' +indir+ 'file.tar ./')
    execute('mkdir tmp')
    train_run("xvf", 'file.tar -C tmp')
    execute('rm -fr file.tar tmp')

    #4
    execute('cp -r ' +indir+ 'file.tar ./')
    train_run("xvf", 'file.tar os2.c os3.c')
    execute('rm -fr file.tar os2.c os3.c os4.c')

    #5
    execute('cp -r ' +indir+ 'file.tar ./')
    execute('cp -r ' +indir+ 'os5.c ./')
    train_run("rvf", 'file.tar os5.c')
    execute('rm -fr file.tar os5.c')

    #6
    execute('cp -r ' +indir+ 'file1.tar ./')
    train_run("tf", 'file1.tar')
    execute('rm -fr file1.tar')

    #7
    execute('cp -r ' +indir+ 'file.tar ./')
    train_run("tvf", 'file.tar os2.c')
    execute('rm -fr file.tar os2.c')

    #8
    execute('cp -r ' +indir+ 'file2.tar ./')
    train_run("tvf", 'file2.tar --wildcards \'*.png\'')
    execute('rm -fr file2.tar')
