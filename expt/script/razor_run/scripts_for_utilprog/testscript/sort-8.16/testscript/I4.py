...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'du_hs_usr_share.txt ./')
    train_run('-nr', 'du_hs_usr_share.txt')
    execute('rm -fr du_hs_usr_share.txt')

    #1
    execute('cp -r ' +indir+ 'ls_l_usr_bin.txt ./')
    train_run('-nr -k 5', 'ls_l_usr_bin.txt')
    execute('rm -fr ls_l_usr_bin.txt')

    #2
    execute('cp -r ' +indir+ 'distros.txt ./')
    train_run('-k 1,1 -k 2n', 'distros.txt')
    execute('rm -fr distros.txt')

    #3
    execute('cp -r ' +indir+ 'distros.txt ./')
    train_run('-k 3.7nbr -k 3.1nbr -k 3.4nbr', 'distros.txt')
    execute('rm -fr distros.txt')

    #4
    execute('cp -r ' +indir+ 'etc_passwd.txt ./')
    train_run('-t \':\' -k 7', 'etc_passwd.txt')
