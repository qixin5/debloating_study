...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'tecmint.txt ./')
    train_run('', 'tecmint.txt')
    execute('rm -fr tecmint.txt')
    ...
    #2
    execute('cp -r ' +indir+ 'tecmint.txt ./')
    train_run('-r', 'tecmint.txt')
    execute('rm -fr tecmint.txt')
    
    #3
    execute('cp -r ' +indir+ 'lsl.txt ./')
    train_run('-nk2', 'lsl.txt')
    execute('rm -fr lsl.txt')
    
    #4
    execute('cp -r ' +indir+ 'lsl.txt ./')
    train_run('-k9', 'lsl.txt')
    execute('rm -fr lsl.txt')
    
    #5
    execute('cp -r ' +indir+ 'lsl.txt ./')
    train_run('-nk5', 'lsl.txt')
    execute('rm -fr lsl.txt')
    
    #6
    execute('cp -r ' +indir+ 'tecmint.txt ./')
    train_run('-u', 'tecmint.txt')
    execute('rm -fr tecmint.txt')
    
    #7
    execute('cp -r ' +indir+ 'lsl.txt ./')
    execute('cp -r ' +indir+ 'lsla.txt ./')    
    train_run('', 'lsl.txt lsla.txt')
    execute('rm -fr lsl.txt lsla.txt')
    
    #8
    execute('cp -r ' +indir+ 'lsl.txt ./')
    execute('cp -r ' +indir+ 'lsla.txt ./')    
    train_run('-u', 'lsl.txt lsla.txt')
    execute('rm -fr lsl.txt lsla.txt')
    
    #9
    execute('cp -r ' +indir+ 'lsl.txt ./')
    train_run('-t "," -nk2,5 -k9', 'lsl.txt')
    execute('rm -fr lsl.txt')
