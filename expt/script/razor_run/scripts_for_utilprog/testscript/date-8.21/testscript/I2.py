...
def train_run_for_i7():
    BIN = './date.orig'
    cmd = 'TZ=\'Australia/Melbourne\' ' + BIN
    cmd = DRRUN + ' -c ' + CLIENT + ' -- ' + cmd
    execute(cmd)
...
def train():
    ...
    #1
    train_run(""" +"Year: %Y, Month: %m, Day: %d" """)

    #2
    train_run(""" "+DATE: %D%nTIME: %T" """)

    #3
    train_run(""" -d "2010-02-07 12:10:53" """)

    #4
    train_run(""" -d '16 Dec 1974' +'%A, %d %B %Y' """)

    #5
    train_run(""" -d "last week" """)

    #6
    train_run(""" -d 'TZ="Australia/Sydney" 06:30 next Monday' """)
    ...
    #9
    train_run(""" -d @1234567890 """)

    #10
    train_run(""" -r /etc/hosts """)
