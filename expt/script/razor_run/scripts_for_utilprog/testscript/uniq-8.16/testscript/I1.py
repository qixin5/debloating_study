...
def train_run_redirect(arg1, arg2):
    BIN = './uniq.orig'
    execute('sort ' + arg2 + ' >sorted.txt')
    cmd = BIN + ' sorted.txt'
    cmd = DRRUN + ' -c ' + CLIENT + ' -- ' + cmd  + ' > tmp.out'
    execute(cmd)
...
def train():
    ...
    train_run("", d + 'authors.txt')

    execute('cp ' +d+ 'authors_unsorted.txt ./')
    execute('sort authors_unsorted.txt >authors_sorted.txt')
    train_run("", 'authors_sorted.txt')
    execute('rm authors_unsorted.txt authors_sorted.txt')
    
    train_run("-c", d + 'authors.txt')
    train_run("-d", d + 'authors.txt')
    train_run("-u", d + 'authors.txt')
    train_run("-s 1", d + 'authors_number_sorted.txt')
    train_run("-f 2", d + 'cricketers.txt')
    train_run("-f 2 -c", d + 'cricketers.txt')
    return
