...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'a.txt ./')
    train_run("""a.txt""")
    execute('rm -fr a.txt')

    #1
    execute('cp -r ' +indir+ 'b.txt ./')
    execute('cp -r ' +indir+ 'c.txt ./')
    train_run("""b.txt c.txt""")
    execute('rm -fr b.txt c.txt')

    #2
    execute('cp -r ' +indir+ 'd.txt ./')
    train_run_with_echo("""-i d.txt""", 'y')
    execute('rm -fr d.txt')

    #3
    execute('cp -r ' +indir+ 'e.txt ./')
    execute('chmod 444 e.txt')
    train_run_with_echo("""e.txt""", 'n')
    execute('chmod 755 e.txt')
    execute('rm -fr e.txt')
