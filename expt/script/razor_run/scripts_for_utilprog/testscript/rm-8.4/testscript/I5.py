...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'voltron.txt ./')
    train_run("""voltron.txt""")
    execute('rm -fr voltron.txt')

    #1
    execute('cp -r ' +indir+ 'decepticons ./')
    train_run("""decepticons""")
    execute('rm -fr decepticons')

    #2
    execute('cp -r ' +indir+ 'decepticons ./')
    train_run("""-r decepticons""")
    execute('rm -fr decepticons')

    #3
    execute('cp -r ' +indir+ 'megatron.txt ./')
    train_run_with_echo("""-i megatron.txt""", 'y')
    execute('rm -fr megatron.txt')

    #4
    execute('cp -r ' +indir+ 'decepticons ./')
    train_run("""-r -v decepticons/""")
    execute('rm -fr decepticons')

    #5
    execute('cp -r ' +indir+ 'unicron.txt ./')
    train_run_with_echo("""unicron.txt""", 'y')
    execute('rm -fr unicron.txt')

    #6
    execute('cp -r ' +indir+ 'unicron.txt ./')
    train_run("""-f unicron.txt""")
    execute('rm -fr unicron.txt')
