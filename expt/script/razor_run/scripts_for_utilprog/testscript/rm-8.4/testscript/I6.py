...
def train():
    ...
    #2
    execute('cp -r ' +indir+ '*.txt ./')
    train_run("""file.txt myfile.txt""")
    execute('rm -fr file.txt myfile.txt')

    #3
    execute('cp -r ' +indir+ '*.txt ./')
    train_run("""-f file.txt myfile.txt""")
    execute('rm -fr file.txt myfile.txt')

    #4
    execute('cp -r ' +indir+ '*.txt ./')
    train_run_with_echo("""-i file.txt myfile.txt""", 'y')
    execute('rm -fr file.txt myfile.txt')
