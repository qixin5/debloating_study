...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'accounts.txt ./')
    train_run("""accounts.txt""")
    execute('rm -fr accounts.txt')

    #1
    execute('cp -r ' +indir+ 'cases ./')
    train_run("""-r cases""")
    execute('rm -fr cases')

    #2
    execute('>/rmtest/newuser/info')
    train_run("""/rmtest/newuser/info""")
    execute('rm -fr /rmtest/newuser/info')

    #3
    execute('cp -r ' +indir+ 'accounts.txt ./')
    execute('cp -r ' +indir+ 'test.txt ./')
    train_run("""accounts.txt test.txt""")
    execute('rm -fr accounts.txt test.txt')
