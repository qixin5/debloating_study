...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" "GNU" GPL-3 """)
    execute('rm -fr GPL-3')

    #1
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" -i "license" GPL-3 """)
    execute('rm -fr GPL-3')

    #2
    execute('cp -r ' +indir+ 'BSD ./')
    train_run(""" -v "the" BSD """)
    execute('rm -fr BSD')

    #3
    execute('cp -r ' +indir+ 'BSD ./')
    train_run(""" -vn "the" BSD """)
    execute('rm -fr BSD')
    ...
    #6
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" "..cept" GPL-3 """)
    execute('rm -fr GPL-3')

    #7
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" "t[wo]o" GPL-3 """)
    execute('rm -fr GPL-3')

    #8
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" "[^c]ode" GPL-3 """)
    execute('rm -fr GPL-3')
    ...
    #10
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" "^[[:upper:]]" GPL-3 """)
    execute('rm -fr GPL-3')

    #11
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" "([A-Za-z ]*)" GPL-3 """)
    execute('rm -fr GPL-3')
    ...
    train_run(""" "\(grouping\)" file.txt """)
    execute('rm -fr file.txt')

    #14
    execute('cp -r ' +indir+ 'file.txt ./')
    train_run(""" -E "(grouping)" file.txt """)
    execute('rm -fr file.txt')

    #15
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" -E "(GPL|General Public License)" GPL-3 """)
    execute('rm -fr GPL-3')

    #16
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" -E "(copy)?right" GPL-3 """)
    execute('rm -fr GPL-3')

    #17
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" -E "free[^[:space:]]+" GPL-3 """)
    execute('rm -fr GPL-3')

    #18
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" -E "[AEIOUaeiou]{3}" GPL-3 """)
    execute('rm -fr GPL-3')

    #19
    execute('cp -r ' +indir+ 'GPL-3 ./')
    train_run(""" -E "[[:alpha:]]{16,20}" GPL-3 """)
    execute('rm -fr GPL-3')
