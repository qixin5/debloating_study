...
def train():
    ...
    #0
    execute('dpkg -l >dpkg_l')
    train_run(""" -i python dpkg_l""")
    execute('rm -fr dpkg_l')
    ...
    execute('find . -name "*.mp3" >dir0findmp3.txt')
    train_run(""" -i JayZ dir0findmp3.txt """)
    execute('rm -fr *.mp3 dir0findmp3.txt')

    #3
    execute('cp -r ' +indir+ 'dir0/* ./')
    execute('{ find . -name "*.mp3" | grep -i JayZ; } >dir0findmp3.txt')
    train_run(""" -vi "remix" dir0findmp3.txt """)
    execute('rm -fr *.mp3 dir0findmp3.txt')

    #4
    execute('cp -r ' +indir+ 'ifconfig_file ./')
    train_run(""" -A 4 eth0 ifconfig_file """)
    execute('rm -fr ifconfig_file')

    #5
    execute('cp -r ' +indir+ 'ifconfig_file ./')
    train_run(""" -B 2 UP ifconfig_file """)
    execute('rm -fr ifconfig_file')

    #6
    execute('cp -r ' +indir+ 'ifconfig_file ./')
    train_run(""" -C 2 lo ifconfig_file """)
    execute('rm -fr ifconfig_file')

    #7
    execute('cp -r ' +indir+ 'ifconfig_file ./')
    train_run(""" -c inet6 ifconfig_file """)
    execute('rm -fr ifconfig_file')

    #8
    execute('cp -r ' +indir+ 'setup..py ./')
    train_run(""" -n "main" setup..py """)
    execute('rm -fr setup..py')

    #9
    execute('cp -r ' +indir+ 'dir1 ./')
    train_run(""" -r "function" dir1 """)
    execute('rm -fr dir1')

    #10
    execute('cp -r ' +indir+ 'ifconfig_file ./')
    train_run(""" -w "RUNNING" ifconfig_file """)
    execute('rm -fr ifconfig_file')

    #11
    execute('cp -r ' +indir+ 'ifconfig_file ./')
    train_run(""" -w "RUN" ifconfig_file """)
    execute('rm -fr ifconfig_file')
