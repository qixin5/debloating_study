...
def train():
    ...
    train_run(""" phoenixNAP sample """)
    execute('rm -fr sample')

    #1
    execute('cp -r ' +indir+ 'sample2 ./')
    train_run(""" phoenix sample2 """)
    execute('rm -fr sample2')

    #2
    execute('cp -r ' +indir+ 'sample* ./')
    train_run(""" phoenix sample sample2 sample3 """)
    execute('rm -fr sample sample2 sample3')

    #3
    execute('cp -r ' +indir+ '* ./')
    train_run(""" nix dir0 sample sample2 sample3 """)
    execute('rm -fr dir0 sample sample2 sample3')

    #4
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -w phoenix dir0 sample sample2 sample3 """)
    execute('rm -fr dir0 sample sample2 sample3')

    #5
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -i phoenix dir0 sample sample2 sample3 """)
    execute('rm -fr dir0 sample sample2 sample3')

    #6
    execute('cp -r ' +indir+ 'dir0 ./')
    train_run(""" -rw phoenix dir0 """)
    execute('rm -fr dir0')

    #7
    execute('cp -r ' +indir+ 'sample ./')
    train_run(""" -vi phoenix sample """)
    execute('rm -fr sample')

    #8
    execute('cp -r ' +indir+ '* ./')
    train_run(""" "phoenix number3" dir0 sample sample2 sample3 """)
    execute('rm -fr dir0 sample sample2 sample3')

    #9
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -x "phoenix number3" dir0 sample sample2 sample3 """)
    execute('rm -fr dir0 sample sample2 sample3')

    #10
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -l phoenix dir0 sample sample2 sample3 """)
    execute('rm -fr dir0 sample sample2 sample3')

    #11
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -c phoenix dir0 sample sample2 sample3 """)
    execute('rm -fr dir0 sample sample2 sample3')

    #12
    execute('cp -r ' +indir+ 'sample ./')
    train_run(""" -n -C 2 Phoenix sample """)
    execute('rm -fr sample')

    #13
    execute('cp -r ' +indir+ 'sample ./')
    train_run(""" -m2 Phoenix sample """)
    execute('rm -fr sample')
