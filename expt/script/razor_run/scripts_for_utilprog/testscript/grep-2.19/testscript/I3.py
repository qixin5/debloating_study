...
def train():
    ...
    #0
    execute('cp -r ' +indir+ 'menu.h ./')
    execute('cp -r ' +indir+ 'main.c ./')
    train_run(""" -i 'hello.*world' menu.h main.c """)
    execute('rm -fr menu.h main.c')

    #1
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -n -- 'f.*\.c$' *g*.h /dev/null """)
    execute('rm -fr argmatch.h dir0 etcmotd file0.c main.c menu.h psef test-*.c test*.log')

    #2
    execute('cp -r ' +indir+ 'test-*.c ./')
    train_run(""" -l 'main' test-*.c """)
    execute('rm -fr test-*.c')

    #3
    execute('cp -r ' +indir+ 'dir0 ./')
    train_run(""" -r 'hello' ./dir0 """)
    execute('rm -fr dir0')

    #4
    execute('cp -r ' +indir+ 'dir0 ./')
    train_run(""" -H 'hello' ./dir0/file0.c """)
    execute('rm -fr dir0')

    #5
    execute('cp -r ' +indir+ 'dir0 ./')
    train_run(""" -H 'hello' ./dir0/*.c """)
    execute('rm -fr dir0')

    #6
    execute('cp -r ' +indir+ 'dir0 ./')
    train_run(""" -r --include='*.c' 'hello' ./dir0 """)
    execute('rm -fr dir0')

    #7
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -- '--cut here--' argmatch.h dir0 etcmotd file0.c main.c menu.h psef test-*.c test*.log """)
    execute('rm -fr argmatch.h dir0 etcmotd file0.c main.c menu.h psef test-*.c test*.log')

    #8
    execute('cp -r ' +indir+ '* ./')
    train_run(""" -e '--cut here--' argmatch.h dir0 etcmotd file0.c main.c menu.h psef test-*.c test*.log """)
    execute('rm -fr argmatch.h dir0 etcmotd file0.c main.c menu.h psef test-*.c test*.log')

    #9
    execute('cp -r ' +indir+ 'test*.log ./')
    train_run(""" -w 'hello' test*.log """)
    execute('rm -fr test*.log')

    #10
    execute('cp -r ' +indir+ 'test*.log ./')
    train_run(""" 'hello\>' test*.log """)
    execute('rm -fr test*.log')

    #11
    execute('cp -r ' +indir+ 'test*.log ./')
    train_run(""" -C 2 'hello' test*.log """)
    execute('rm -fr test*.log')

    #12
    train_run(""" qxin6 /etc/passwd /dev/null """)

    #13
    train_run(""" -H qxin6 /etc/passwd /dev/null """)

    #14
    execute('cp -r ' +indir+ 'psef ./')
    train_run(""" '[c]ron' psef """)
    execute('rm -fr psef')

    #15
    execute('cp -r ' +indir+ 'etcmotd ./')
    train_run(""" 'paul' etcmotd """)
    execute('rm -fr etcmotd')

    #16
    execute('cp -r ' +indir+ 'etcmotd ./')
    execute('grep \'paul\' etcmotd >etcmotd_paul.txt')
    train_run(""" 'franc,ois' etcmotd_paul.txt """)
    execute('rm -fr etcmotd etcmotd_paul.txt')

    #17
    execute('cp -r ' +indir+ 'etcmotd ./')
    train_run(""" 'qxin6' /etc/passwd etcmotd """)
    execute('rm -fr etcmotd')

    #18
    execute('echo \'ba\' >ba')
    train_run(""" -E '(a)\1|b\1' ba """)
    execute('rm -fr ba')

    #19
    execute('printf \'foo\nbar\n\' >foobar')
    train_run(""" -z -q 'foo[[:space:]]\+bar' foobar """)
    execute('rm -fr foobar')
