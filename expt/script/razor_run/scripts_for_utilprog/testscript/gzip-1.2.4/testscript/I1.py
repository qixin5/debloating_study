...
def train():
    ...
    #0
    execute('cp -r ' +d+ 'file.txt ./')
    train_run("", 'file.txt')
    execute('rm -fr file.txt file.txt.gz')

    #1
    execute('cp -r ' +d+ 'file1.txt ./')
    execute('cp -r ' +d+ 'file2.txt ./')
    execute('cp -r ' +d+ 'file3.txt ./')
    train_run("", 'file1.txt file2.txt file3.txt')
    execute('rm -fr file1.txt file2.txt file3.txt file1.txt.gz file2.txt.gz file3.txt.gz')

    #2
    execute('cp -r ' +d+ 'file.txt ./')
    train_run("-c", 'file.txt')
    execute('rm -fr file.txt file.txt.gz')

    #3
    execute('cp -r ' +d+ 'example ./')
    execute('cp -r ' +d+ 'file1.txt ./')
    execute('cp -r ' +d+ 'file2.txt ./')
    execute('cp -r ' +d+ 'file3.txt ./')
    train_run("-r", 'example file1.txt file2.txt file3.txt')
    execute('rm -fr example file1.txt file2.txt file3.txt file1.txt.gz file2.txt.gz file3.txt.gz')

    #4
    execute('cp -r ' +d+ 'file.txt ./')
    execute('gzip file.txt')
    train_run("-d", 'file.txt.gz')
    execute('rm -fr file.txt.gz file.txt')

    #5
    execute('cp -r ' +d+ 'linux-3.18.19.tar ./')
    execute('gzip linux-3.18.19.tar')
    train_run("-l", 'linux-3.18.19.tar.gz')
    execute('rm -fr linux-3.18.19.tar linux-3.18.19.tar.gz')

    #6
    execute('cp -r ' +d+ 'linux-3.18.19.tar ./')
    train_run("-1", 'linux-3.18.19.tar')
    execute('rm -fr linux-3.18.19.tar linux-3.18.19.tar.gz')

    #7
    execute('cp -r ' +d+ 'linux-3.18.19.tar ./')
    train_run("-9", 'linux-3.18.19.tar')
    execute('rm -fr linux-3.18.19.tar linux-3.18.19.tar.gz')

    #8
    execute('cp -r ' +d+ 'file1.txt ./')
    execute('gzip file1.txt')
    train_run("-tv", 'file1.txt.gz')
    execute('rm -fr file1.txt file1.txt.gz')

    #9
    execute('cp -r ' +d+ 'file1_corrupted.txt.gz ./')
    train_run("-tv", 'file1_corrupted.txt.gz')
    execute('rm -fr file1_corrupted.txt.gz')

    #10
    execute('cp -r ' +d+ 'file.txt.gz ./')
    train_run("-lv", 'file.txt.gz')
    execute('rm -fr file.txt.gz file.txt')

    #11
    execute('cp -r ' +d+ 'file2.txt ./')
    train_run("-c", 'file2.txt')
    execute('rm -fr file2.txt file2.txt.gz')
