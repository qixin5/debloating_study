...
def train():
    ...
    #0
    execute('cp -r ' +d+ 'test.txt ./')
    train_run("", 'test.txt')
    execute('rm -fr test.txt test.txt.gz')

    #1
    execute('cp -r ' +d+ 'test.txt ./')
    train_run("-v", 'test.txt')
    execute('rm -fr test.txt test.txt.gz')

    #2
    execute('cp -r ' +d+ 'test1.txt ./')
    execute('cp -r ' +d+ 'test2.txt ./')
    execute('cp -r ' +d+ 'test3.txt ./')
    execute('cp -r ' +d+ 'test4.txt ./')
    train_run("", 'test1.txt test2.txt test3.txt test4.txt')
    execute('rm -fr test1.txt.gz test2.txt.gz test3.txt.gz test4.txt.gz test1.txt test2.txt test3.txt test4.txt')

    #3
    execute('cp -r ' +d+ 'test.txt.gz ./')
    train_run("-l", 'test.txt.gz')
    execute('rm -fr test.txt.gz test.txt')
    ...
    #5
    execute('cp -r ' +d+ 'test.txt ./')
    train_run("-1", 'test.txt')
    execute('rm -fr test.txt test.txt.gz')

    #6
    execute('cp -r ' +d+ 'test.txt ./')
    train_run("-9", 'test.txt')
    execute('rm -fr test.txt test.txt.gz')

    #7
    execute('cp -r ' +d+ 'test.txt.gz ./')
    train_run("-d", 'test.txt.gz')
    execute('rm -fr test.txt.gz test.txt')
