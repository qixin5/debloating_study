...
def train():
    ...
    #0
    execute('cp -r ' +d+ 'dump.doc ./')
    execute('cp -r ' +d+ 'file1 ./')
    execute('cp -r ' +d+ 'test.sh ./')
    train_run("", 'dump.doc file1 test.sh')
    execute('rm -fr dump.doc.gz file1.gz test.sh.gz dump.doc file1 test.sh')

    #1
    execute('cp -r ' +d+ 'test ./')
    execute('tar cf test.tar test')
    train_run("", 'test.tar')
    execute('rm -fr test.tar test.tar.gz')

    #2
    execute('cp -r ' +d+ 'dump.doc ./')
    execute('cp -r ' +d+ 'file1 ./')
    execute('cp -r ' +d+ 'test.sh ./')
    execute('cat dump.doc file1 test.sh | gzip > all.gz')
    train_run("-l", 'all.gz')
    execute('rm -fr all.gz dump.doc file1 test.sh')
