...
def train():
    ...
    #0
    execute('cp -r ' +d+ 'helloworld ./')
    train_run("", 'helloworld')
    execute('rm -fr helloworld helloworld.gz')
    
    #1
    execute('cp -r ' +d+ 'helloworld ./')
    execute('gzip helloworld')
    train_run("-l", 'helloworld.gz')
    execute('rm -fr helloworld helloworld.gz')
    
    #2
    execute('cp -r ' +d+ 'helloworld ./')
    train_run("-9", 'helloworld')
    execute('rm -fr helloworld helloworld.gz')
    
    #3
    execute('cp -r ' +d+ 'helloworld ./')
    execute('gzip helloworld')
    train_run("-d", 'helloworld.gz')
    execute('rm -fr helloworld.gz helloworld')
    
    #4
    execute('cp -r ' +d+ 'helloworld ./')
    execute('gzip helloworld')
    train_run("-dk", 'helloworld.gz')
    execute('rm -fr helloworld.gz helloworld')
    
    #5
    execute('cp -r ' +d+ 'helloworld ./')
    execute('cp -r ' +d+ 'javaselightbox ./')    
    train_run("-r", 'helloworld javaselightbox')
    execute('rm -fr helloworld.gz javaselightbox.gz helloworld javaselightbox')
    
    #6
    execute('cp -r ' +d+ 'helloworld ./')
    execute('cp -r ' +d+ 'javaselightbox ./')    
    execute('gzip helloworld')
    execute('gzip javaselightbox')
    train_run("-dr", 'helloworld.gz javaselightbox.gz')
    execute('rm -fr helloworld.gz javaselightbox.gz helloworld javaselightbox')
