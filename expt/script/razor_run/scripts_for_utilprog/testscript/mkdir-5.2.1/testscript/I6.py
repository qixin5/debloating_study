...
def train():
    ...
    train_run("", "sysadmin")
    execute('rm -fr sysadmin')

    train_run("", "/tmp/backup")
    execute('rm -fr /tmp/backup')

    train_run("", "codedb sysdb backupdb")
    execute('rm -fr codedb sysdb backupdb')

    train_run("", "/tmp/{codedb,sysdb,backupdb}")
    execute('rm -fr /tmp/codedb /tmp/sysdb /tmp/backupdb')

    train_run("", "/home/qxin6/tmp/sysadmins/engineers")
    execute('rm -fr /home/qxin6/tmp/sysadmins')

    train_run("-p", "/home/qxin6/tmp/sysadmins/engineers")
    execute('rm -fr /home/qxin6/tmp/sysadmins')

    train_run("-p", "/tmp/lab01/{expr1,expr2} /tmp/lab02/{expr3,expr4}")
    execute('rm -fr /tmp/lab01 /tmp/lab02')

    train_run("-m 700", "/tmp/shared")
    execute('rm -fr /tmp/shared')

    train_run("-v", "/tmp/pkumar")
    execute('rm -fr /tmp/pkumar')

    train_run("-v", "/tmp/pkumar/{dir1,dir2,dir3}")
    execute('rm -fr /tmp/pkumar')

    train_run("-Z", "/tmp/linuxbuzz")
    execute('rm -fr /tmp/linuxbuzz')
