...
$OUT_DIR = $ARGV[1];
$TIMEOUT = $ARGV[2];
$scriptPath = $ARGV[3];   #This is path to the fuzzed input (e.g., move#4.1.1...)
...
#						print(">>>>>>>>running test ".($scriptName)."\n");
						...
						`cd $TEST_DIR/$level1/$level2 && cat $SETUP_SCRIPT_PREFIX$fileParts[1] $scriptPath $CLEANUP_SCRIPT_PREFIX$fileParts[2] > $IN_DIR/$SCRIPT_FILE && chmod $FILE_PERMS $IN_DIR/$SCRIPT_FILE`;
						$outfile = "$OUT_DIR/$OUT_ID";
				
						`timeout -k 9 ${TIMEOUT}s $IN_DIR/$SCRIPT_FILE > $outfile 2>&1`;
				
						$num = `ls *.out | wc -l`;
						if($num == 0)
						{
						    #`mv $IN_DIR/copy1 $outfile`;
						    `cat $IN_DIR/copy1 >>$outfile`
						}
						else
						{
						    #`mv $fileParts[0].$fileParts[1].$fileParts[2].out $outfile`;
						    `cat $fileParts[0].$fileParts[1].$fileParts[2].out >>$outfile`;
						}
