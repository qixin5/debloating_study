...
if [ ${force_to_run_origin} == "true" ]; then
    ...
    origin_output_dir=../debaug/$BENCH/$PROGNAME/output/origin/${testid}
...
if [ ${need_to_run_origin} == "true" ]; then
    test/run_test.sh $BENCH $PROGNAME origin ${testid}
fi
test/run_test.sh $BENCH $PROGNAME ${progpath} ${testid}
...
for output_f1 in ./output/origin/${testid}/*; do
    fname=$(basename ${output_f1})
    if [ ${progpath} == "origin" ]; then
	echo "pass-${fname}" >>compare_rslt.txt
    else
	if diff -q ${output_f1} ./output/reduced/${progpath}/${testid}/${fname} &>/dev/null; then
            echo "pass-${fname}" >>compare_rslt.txt
	else
