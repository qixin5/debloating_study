    ...
    if (ctx->buflen != 0U) {
      ...
      if (ctx->buflen > 64U) {
        ...
        memcpy((void *)(ctx->buffer),
               (void const *)((char *)(ctx->buffer) +
                              ((left_over + add) & 0xffffffffffffffc0UL)),
               (size_t)ctx->buflen);
      }
...
void usage(int status) {
...

  {
    if (status != 0) {
      ...
      tmp___14 =
          gettext("  -t, --field-separator=SEP  use SEP instead of non-blank "
    ...
    if ((unsigned int)e == 0U) {
      ...
      if ((uintmax_t)nmerge != n) {
        ...
        if (nmerge < 2U) {
          ...
          error(0, 0, (char const *)tmp___4, long_options___1[oi].name,
                tmp___3);
          tmp___5 = quote("2");
          tmp___6 = gettext("minimum --%s argument is %s");
          error(2, 0, (char const *)tmp___6, long_options___1[oi].name,
                tmp___5);
        } else {
                    ...
                    if (key->human_numeric) {
                      ...
                      if (key->month) {
                        ...
                        if (key->random) {
                          ...
                          if (key->version) {
                            diff =
                                filevercmp((char const *)ta, (char const *)tb);
                          } else {
                            if (tlena == 0UL) {
                              diff = -(tlenb != 0UL);
                            } else {
